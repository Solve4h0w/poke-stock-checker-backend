...
// for ONE tcin at ONE store. We spoof your browser headers/cookies,
// decompress gzip if needed, parse JSON, and return a clean summary.
//
// SECURITY NOTE:
// - COOKIE_BLOB is still inlined. Move to env vars later.
// - store/location details are from your Richland WA store for now.
//
// After this runs, /api/target/store/:tcin will give you:
// { ok, tcin, storeName, status, qty, ... }

import { request } from "undici";
import { gunzipSync, brotliDecompressSync, inflateSync } from "node:zlib";

// ---------- CONSTANTS FROM YOUR SESSION ----------
...
// API key param Target used
const KEY = "9f36aeafbe60771e321a7cc95a78140772ab3e96";


// Build Target RedSky URL for this tcin
...
// Build headers to look like your browser
...
// Safely decompress based on header
function decompressBody(buffer, encoding) {
  ...
  try {
    ...
    // fall through, we'll just return what we have
    console.error("[decompressBody] failed:", err);
  }
...
export async function fetchAvailability({ tcin }) {
  ...
  const decBuf = decompressBody(rawBuf, contentEncoding);

  // 5. Convert bytes -> text
  const text = new TextDecoder("utf-8").decode(decBuf);
  ...
  const storeName =
    firstStore.location_name ||
    fulfillment.location_name ||
    "Unknown store";

  const postalCode =
    firstStore?.mailing_address?.postal_code ||
    fulfillment?.mailing_address?.postal_code ||
    ZIP;
  ...
  const pickupStatus =
    firstStore.order_pickup?.availability_status ??
    firstStore.availability_status ??
    "UNKNOWN";

  const shipToStoreStatus =
    firstStore.ship_to_store?.availability_status ??
    "UNKNOWN";

  // Timestamp (server-side now)
  const updated = new Date().toISOString();

  // 8. Return clean object
  return {
    ok: true,
    status,
    tcin,
    ...
    raw: json, // keep raw for now so we can inspect; remove later if you want
  };
}
