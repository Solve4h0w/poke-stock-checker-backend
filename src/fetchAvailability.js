...
// ---------------------------------------------
// Fetch live Target RedSky inventory for a single TCIN + store.
// Handles gzip / brotli / deflate decompression.
// Returns a clean summary for your frontend.
// ---------------------------------------------
...
// Full cookie string from your session
...
function buildHeaders(tcin) {
  return {
    ...
    "sec-ch-ua": `"Google Chrome";v="141", "Chromium";v="141"`,
    "sec-ch-ua-mobile": "?0",
...
export async function fetchAvailability({ tcin }) {
  ...
  const decoded = decompress(raw, enc);
  ...
  try {
    ...
    return { ok: false, status, error: "Bad JSON from Target", snippet: text.slice(0, 400) };
  }
  ...
  const storeOptions = Array.isArray(fulfillment.store_options)
    ...
    : [];
  const store = storeOptions[0] || {};

  const storeId = store.location_id || STORE_ID;
  const storeName =
    store.store_name ||
    store.location_name ||
    ...
    "Unknown store";
  const postal = store?.mailing_address?.postal_code || ZIP;
  const statusText = store.availability_status || fulfillment.availability_status || "UNKNOWN";
  const qty =
    store.location_available_to_promise_quantity ??
    store.available_to_promise_quantity ??
    0;
  const pickup =
    store.order_pickup?.availability_status ??
    store.availability_status ??
    "UNKNOWN";
  const shipToStore =
    store.ship_to_store?.availability_status ??
    "UNKNOWN";
  ...
  return {
    ...
    debug: { contentEncoding: enc || null, decodedOK: true },
    raw: json,
  };
}
