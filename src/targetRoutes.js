...
//
// Express routes exposing Target data.
// Right now you get:
//   GET /api/target/ping
//   GET /api/target/store/:tcin   <- live per-store availability for one Target item
//
// Later we can generalize the store info and move secrets (cookie, visitorId)
// to environment variables instead of hardcoding them.
...
/**
 * Simple ping/debug route so you can confirm routes are mounted.
 * GET /api/target/ping
 */
...
/**
 * GET /api/target/store/:tcin
 *
 * Example:
 *   /api/target/store/93954446
 *
 * What it does:
 *  - Uses values you captured from your own Target session (Richland WA store)
 *  - Calls fetchAvailability() which talks to RedSky
 *  - Returns cleaned data (ok/status/tcin/storeId/raw...)
 *
 * Response on success:
 * {
 *   "ok": true,
 ...
 *   "ok": false,
 *   "status": 502,
 *   "error": "RedSky returned 502",
 *   "snippet": "{... first ~500 chars of body ... }"
 * }
 */
router.get("/store/:tcin", async (req, res) => {
  const { tcin } = req.params || {};

  if (!tcin) {
    return res
      .status(400)
      .json({ ok: false, error: "Missing tcin (Target product ID)" });
  }

  // ─────────────────────────────────────────────
  // HARD-CODED STORE CONTEXT FOR NOW
  //
  // All of these were captured from your own browser DevTools
  // while looking at this product in the Richland WA store.
  //
  // Eventually:
  //  - These move to environment variables
  //  - We allow ?storeId=... etc for other stores / nationwide
  // ─────────────────────────────────────────────
  ...
  try {
    ...
    // if Target said "nope", bubble up that status so you see it
    if (!result.ok) {
    ...
    console.error("[/api/target/store/:tcin] error:", err);
